...
router.get('/', async (req, res) => {
  try {
    ...
    if (!positions) {
      res
        .status(400)
        .json({ msg: 'There are no positions available at the moment' });
    }
...
router.post(
  ...
  [
    ...
    [
      ...
      check('venue', 'Please include the id of the venue related to the job')
        .not()
        .isEmpty(),
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!venueObject) {
        return res.status(400).json({ msg: 'There is no venue with that id.' });
      }
...
router.put('/:id', authAdminAccess, async (req, res) => {
  try {
    ...
    position = await Position.findByIdAndUpdate(req.params.id, updatedFields, {
      new: true,
    });
